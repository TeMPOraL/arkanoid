...
namespace ARK
{
	...
	//	numEntries = 1;
		//===
		ARK::log <<"CStringTable::Init() - initializing from file " << fN <<"\n";
		FILE *fIn;	//string table file
		if(! (fIn = fopen(fN,"rb")))	//if failed to open in binary mode
		{
			ARK::log.Error("CStringTable::Init() - Error opening file %s!",fN);
			//error logging
			return false;
		}
		if(!LoadTable(fIn))
		{
			ARK::log.Error("CStringTable::Init() - Error loading file %s!",fN);
			return false;
		}
...
#ifdef __ARKANOID_FULL_DEBUG__
		ARK::log <<"CStringTable::LoadTable() - number of string entries = " << numStringEntries << "\n";
#endif //__ARKANOID_FULL_DEBUG__
		if(numStringEntries  <=0)
		{
			ARK::log.Error("CStringTable::LoadTable() - number of entries is greater or equal to 0; nmuber of entries: %d", numStringEntries);
			//error logging
			return false;
		}
...
#ifdef __ARKANOID_FULL_DEBUG__
		ARK::log <<"CStringTable::LoadTable() - loaded strings\n";
#endif //__ARKANOID_FULL_DEBUG__
