...
namespace ARK
{
	...
	bool CWindow::Init( int _width, int _height, int _bpp, bool fullscreen)
	{
		...
		std::string name(CGame::GetSingleton().systemStrings.GetString(STRING_SYS_GAMENAME));
		if(CGame::GetSingleton().bDebugMode == true)
		{
			name += " [ ";
			name += CGame::GetSingleton().runParams;
			name += " ]";
		}
	...
	//	InitWindowClass();	//initialize Window class
		...
		{	
			...
			if (ChangeDisplaySettings(&dmScreenSettings,CDS_FULLSCREEN)!=DISP_CHANGE_SUCCESSFUL)
			{
				...
				if (MessageBox(NULL,(CGame::GetSingleton().systemStrings.GetString(STRING_SYS_VIDEO_MODE_NOT_SUPPORTED)).c_str(),(CGame::GetSingleton().systemStrings.GetString(STRING_SYS_GAMENAME)).c_str(),MB_YESNO|MB_ICONEXCLAMATION)==IDYES)
				{
				...
				{
					MessageBox(NULL,(CGame::GetSingleton().systemStrings.GetString(STRING_SYS_PROGRAM_WILL_NOW_CLOSE)).c_str(),(CGame::GetSingleton().systemStrings.GetString(STRING_SYS_GAMENAME)).c_str(),MB_OK|MB_ICONSTOP);
		...
		if(!(hWnd = CreateWindowEx(NULL, name.c_str(),name.c_str(), dwStyle | WS_CLIPCHILDREN | WS_CLIPSIBLINGS, 0, 0, width, height, NULL, NULL, hInstance, NULL)))
		{
			...
			MessageBox(NULL,(CGame::GetSingleton().systemStrings.GetString(STRING_SYS_FAILED_CREATING_WINDOW)).c_str(),(CGame::GetSingleton().systemStrings.GetString(STRING_SYS_GAMENAME)).c_str(),MB_OK|MB_ICONSTOP);
		...
		if(! (hDC = ::GetDC(hWnd) ) )	//aquiring Device Context
		{
			...
			MessageBox(NULL,(CGame::GetSingleton().systemStrings.GetString(STRING_SYS_FAILED_AQUIRING_DC)).c_str(),(CGame::GetSingleton().systemStrings.GetString(STRING_SYS_GAMENAME)).c_str(),MB_OK|MB_ICONSTOP);
		...
		if(!(PixelFormat=ChoosePixelFormat(hDC,&pfd)))	//choosing Pixel Format
		{
			...
			MessageBox(NULL,(CGame::GetSingleton().systemStrings.GetString(STRING_SYS_FAILED_CHOOSING_PIXELFORMAT)).c_str(),(CGame::GetSingleton().systemStrings.GetString(STRING_SYS_GAMENAME)).c_str(),MB_OK|MB_ICONSTOP);
		...
		if(! ( SetPixelFormat(hDC,PixelFormat,&pfd) ) )	//set selected PixelFormat
		{
			...
			MessageBox(NULL,(CGame::GetSingleton().systemStrings.GetString(STRING_SYS_FAILED_SETTING_PIXELFORMAT)).c_str(),(CGame::GetSingleton().systemStrings.GetString(STRING_SYS_GAMENAME)).c_str(),MB_OK|MB_ICONSTOP);
		...
		if(! (hRC = wglCreateContext(hDC) ) )	//creating rendering context
		{
			...
			MessageBox(NULL,(CGame::GetSingleton().systemStrings.GetString(STRING_SYS_FAILED_CREATING_RC)).c_str(),(CGame::GetSingleton().systemStrings.GetString(STRING_SYS_GAMENAME)).c_str(),MB_OK|MB_ICONSTOP);
		...
		if( !wglMakeCurrent(hDC,hRC) )	//activate rendering context
		{
			...
			MessageBox(NULL,(CGame::GetSingleton().systemStrings.GetString(STRING_SYS_FAILED_ACTIVATING_RC)).c_str(),(CGame::GetSingleton().systemStrings.GetString(STRING_SYS_GAMENAME)).c_str(),MB_OK|MB_ICONSTOP);
		...
		if ( !InitGL() )	//init window-related GL stuff
		{
			...
			MessageBox(NULL,(CGame::GetSingleton().systemStrings.GetString(STRING_SYS_ERROR_INITIALIZING_OPENGL)).c_str(),(CGame::GetSingleton().systemStrings.GetString(STRING_SYS_GAMENAME)).c_str(),MB_OK|MB_ICONSTOP);
	...
	bool CWindow::InitGL()
	{
		...
		glClearColor(0.0f, 1.0f, 0.0f, 0.0f);	//screen clear color
	...
	void CWindow::Resize(int newWidth, int newHeight)
	{
		...
		//gluPerspective(45.0f,(GLfloat)width/(GLfloat)height,0.1f,100.0f);	//calculate perspective
...
LRESULT CALLBACK UniversalWndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	return ARK::CGame::GetSingleton().gameWindow.WndProc(hWnd, msg, wParam, lParam);
}